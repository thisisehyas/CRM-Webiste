...
const ProductsCols = (props) => {
  ...
  const fetchMoreData = () => {
    ...
    let apiUrl = `http://127.0.0.1:8000/machine/?_start=${products.length}&_limit=6`;

    // If search term is provided, add it to the API URL
    if (searchTerm.trim() !== "") {
      apiUrl = `http://127.0.0.1:8000/machine/?search=${searchTerm}&_start=${products.length}&_limit=6`;
      setProducts([]);
    }
